...
- 五种分析模式（顶部“功能选择”切换）：
  ...
  6. **一键分析（自动报告）**：按当前制程模板依次运行 UPH/EFF/报警/机台状态 4 项分析，导出 4 个 Excel，并基于 `Analysis_Report.pptx` 自动生成汇总 PPT 报告（含各分析 summary 与图表）；文档合并与内容拆分保留为独立功能，单独选择执行（大日志制程可显著提速）
