...
- 已配置制程：LM（单颗循环按运动节拍拆分：打标前间隔+Z轴焦距定位+激光打标(振镜扫描)，段和≈CT；批次级 读码GetSN/CCD轴移动定位 每批6颗 standalone；盘级 平台下料/上料 每盘24颗 standalone；**换盘间隔**（下料完成→上料请求，standalone，层级=换盘，甘特图上浅灰蓝条补齐换盘过程））、FR（左/右轴点胶=整循环，日志秒级精度无法细分）、SA（4工位行周期 + 左右点胶头内部动作 视觉对位/探针对位/点胶轮廓）、ACF（上料/主机/下料）、CAW（取料1(工位4)/取料2(工位7)：取料=取熟料完成,双取生料完成 → 扫码完成 → 放料完成）。
