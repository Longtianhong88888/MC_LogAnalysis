...
- **单颗循环步骤分析**（`analyze_steps`，输出到 UPH_Analysis.xlsx 的「步骤分析」sheet）：
  ...
  - 已配置制程：LM（单颗循环按运动节拍拆分：打标前间隔+Z轴焦距定位+激光打标(振镜扫描)，段和≈CT；批次级 读码GetSN/CCD轴移动定位 每批6颗 standalone；盘级 平台下料/上料 每盘24颗 standalone）、FR（左/右轴点胶）、SA（点胶/贴附/热压/检测）、ACF（上料/主机/下料）、CAW（取料1(工位4)/取料2(工位7)：取料=取熟料完成,双取生料完成 → 扫码完成 → 放料完成）。
