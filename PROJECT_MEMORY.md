...
- **单颗循环步骤分析**（`analyze_steps`，输出到 UPH_Analysis.xlsx 的「步骤分析」sheet）：
  ...
  - 已配置制程：LM（打标前间隔+激光打标=单颗循环，段和≈CT；读码GetSN/CCD取像解析/CCD定位 为每盘一次 standalone）、FR（左/右轴点胶）、SA（点胶/贴附/热压/检测）、ACF（上料/主机/下料）、CAW（取料1(工位4)/取料2(工位7)：取料=取熟料完成,双取生料完成 → 扫码完成 → 放料完成）。
...
- **LM**：旧 B 模式把每盘一次的读码/CCD 混进单颗循环，CCD取像解析伪段 1.8s、步骤和 3.36s vs CT 1.39s。已改为 打标前间隔(B:MarkEnd1→MarkStart1)+激光打标(A:MarkStart1→MarkEnd1) 单颗循环，段和 1.389s vs CT 1.393s（-0.3%）✓；读码/CCD 改 standalone（读码中位 0.128s，循环数 4861≈每盘 6 颗）。
- **FR**：左/右轴点胶 3.00s = CT ✓。
...
- `venv/bin/python -m unittest discover -s tests`，66 个用例全绿。
