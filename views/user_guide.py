...
<ol>
  ...
  <li>一键分析（自动报告）只运行 UPH / EFF / 报警 / 状态 4 项并导出 Excel（不含 PPT）；文档合并与内容拆分需单独选择执行，避免大日志拖慢整体速度。</li>
  <li><b>自动报告</b>：一键运行分析，结果直接显示在界面并导出 Excel 报告。</li>
  <li><b>设备效能</b>：主窗口顶部独立页签，页面内选站位与机台号（均支持下拉选择或直接输入，机台号可逗号分隔多台，留空=该站位全部），选站位后机台号自动从数据库更新；开始/结束时间用日历选择（点下拉箭头弹日历，时间可直接修改）；先「查询数据」预览，确认有数据后「输出报告」，以联网数据生成 UPH / EFF / 报警 / 状态 四张 Excel 与 PPT。</li>
</ol>
